...
$guia      = get_object_vars($array_llegada->guia);
...
crear_xml($path, $nombre_archivo, $empresa, $guia, $detalle);

//$this->firmar_xml($nombre_archivo.".xml", $empresa['modo']);
//
//$token_access = $this->token($empresa['guias_client_id'], $empresa['guias_client_secret'], $empresa['usu_secundario_produccion_user'], $empresa['usu_secundario_produccion_password']);
//$respuesta = $this->envio_xml($path.'FIRMA/', $nombre_archivo, $token_access);
//$data_ticket = $respuesta->numTicket;
//
//$respuesta_ticket = $this->envio_ticket($path.'CDR/', $data_ticket, $token_access, $empresa['ruc'], $nombre_archivo);
//$jsondata = array(
...
function crear_xml($path, $nombre_archivo, $empresa, $guia, $detalle){
